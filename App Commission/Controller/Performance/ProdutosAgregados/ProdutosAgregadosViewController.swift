//
//  ProdutosAgregadosViewController.swift
//  App Commission
//
...
struct ListItemTabela {
    let chave: String
    let valor: Int
}

class ProdutosAgregadosViewController: UIViewController, UIScrollViewDelegate {
    ...
    var volumePenetrationProdutosAgregados: [ListItemTabela] = []
    ...
    override func viewDidLoad() {
        ...
        self.getPeriods()
        self.getPerformanceCustom(isFrota: self.isFrota, tipoPerformance: self.tipoPerformance, isAcumulado: self.isAcumulado, actualPeriod: nil, cd_dn: nil, nm_grp_comission: nil)
        ...
        {
            self.tituloTabela.text = "ISR Período Vigente"
        }
        else
        {
            self.tituloTabela.text = "Volume Produtos Agregados"
        }
    ...
    @objc func handleTapOnIsr(sender: UITapGestureRecognizer) {
        ...
        if (self.countIsr == 0)
        {
            ...
            self.tituloTabela.text = self.tipoPerformance.text
    ...
    @objc func handleTapOnProdutos(sender: UITapGestureRecognizer) {
        ...
        if (self.countProduto == 0) {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
                ...
                self.cd_dn = self.nm_grp_comission
                
                self.getPerformanceCustom(isFrota: self.isFrota, tipoPerformance: self.tipoPerformance, isAcumulado: self.isAcumulado, actualPeriod: nil, cd_dn: self.cd_dn, nm_grp_comission: self.nm_grp_comission)
    ...
    @objc func linkFrotaTapped() {
        ...
        if (tipoPerformance.text == "ISR Período Vigente")
        {
            ...
            let performanceProdutosAgregados = ProdutosAgregadosFrotaViewController(isFrota: true, tipoPerformance: tipoPerformanceLabel, isAcumulado: self.isAcumulado)
    ...
    private func getPerformanceCustom(isFrota: Bool, tipoPerformance: UILabel, isAcumulado: Bool, actualPeriod: String?, cd_dn: String?, nm_grp_comission: String?) {
        ...
        default: //Produtos Agregados
            ...
            VolumePenetrationService.shared.getVolumeProdutosAgregados(isFrota: isFrota, actualPeriod: actualPeriod, nm_grp_comission: nm_grp_comission, cd_dn: cd_dn) { volumePerformance, error in
                ...
                if let volumePerformance = volumePerformance {
                    ...
                    self.volumePenetrationProdutosAgregados.append(ListItemTabela(chave: "Acidentes Pessoais", valor: valorAcidentesPessoais))
                    
                    let valorFranquia = volumePerformance.franquia ?? 0
                    self.volumePenetrationProdutosAgregados.append(ListItemTabela(chave: "Franquia", valor: valorFranquia))
                    
                    let valorGap = volumePerformance.gap ?? 0
                    self.volumePenetrationProdutosAgregados.append(ListItemTabela(chave: "Gap", valor: valorGap))
                    
                    let valorGarantia = volumePerformance.garantiaExtendida ?? 0
                    self.volumePenetrationProdutosAgregados.append(ListItemTabela(chave: "Garantia Extendida", valor: valorGarantia))
                    
                    let valorPps = volumePerformance.pps ?? 0
                    self.volumePenetrationProdutosAgregados.append(ListItemTabela(chave: "PPS", valor: valorPps))
                    
                    let valorSeguro = volumePerformance.seguroCasco ?? 0
                    self.volumePenetrationProdutosAgregados.append(ListItemTabela(chave: "Seguro Casco", valor: valorSeguro))
                    
                    let valorSpf = volumePerformance.spf ?? 0
                    self.volumePenetrationProdutosAgregados.append(ListItemTabela(chave: "SPF", valor: valorSpf))
...
extension ProdutosAgregadosViewController: UITableViewDelegate, UITableViewDataSource, UIPickerViewDataSource, UIPickerViewDelegate {
        ...
        case 1:
            ...
            if (tipoPerformance.text == "ISR Período Vigente")
            {
                ...
                if isAcumuladoSelected {
                    ...
                    self.getPerformanceCustom(isFrota: self.isFrota, tipoPerformance: self.tipoPerformance, isAcumulado: isAcumuladoSelected, actualPeriod: periodo, cd_dn: nil, nm_grp_comission: nil)
                } else {
                    self.getPerformanceCustom(isFrota: self.isFrota, tipoPerformance: self.tipoPerformance, isAcumulado: isAcumuladoSelected, actualPeriod: selectedPeriod, cd_dn: nil, nm_grp_comission: nil)
                }
            ...
            else
            {
                ...
                self.getPerformanceCustom(isFrota: self.isFrota, tipoPerformance: self.tipoPerformance, isAcumulado: false, actualPeriod: nil, cd_dn: selectedCdDn, nm_grp_comission: nil)
        ...
        if keyboarIsOpen {
            ...
            if tableView.cellForRow(at: indexPath)?.tag == 100 {
                if indexPath.section - 1 == 3 {
                    ...
                    vc.titulo = "Garantia Estendida - Detalhe"
                ...
                if indexPath.section - 1 == 5 {
                    ...
                    vc.garantiaExtendidaList = nil
                    vc.grpComission = self.periodsSelectField.text
                    vc.titulo = "Seguro Casco - Detalhe"
                    self.navigationController?.pushViewController(vc, animated: true)
                }
